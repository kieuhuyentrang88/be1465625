...
app.post("/api1465625/books", async (req, res) => {
  const { title, author } = req.body;
  console.info(`Adding book with title ${title} by author ${author}`);

  const result = await query(
    "INSERT INTO tblbookIUH1465625 (title, author) VALUES ($1, $2) RETURNING id",
    [title, author]
  );
...
app.get("/api1465625/books", async (req, res) => {
  ...
  const books = await query("SELECT * FROM tblbookIUH1465625");
  res.json(books);
});
